...
class AccidentPredictor:
    ...
    def predict(self, user_data: dict):
        ...
        internal_data = {
            ...
            "vitesse_x_collision": user_data["vitesse_max_autorisee"] if user_data["collision_frontale"] else 0,
            "age_x_securite": user_data["age_usager"] if user_data["ceinture_ou_casque_attache"] else 0,
            "agglo_x_vitesse": user_data["vitesse_max_autorisee"] if user_data["en_agglomeration"] else 0,
        ...
        return {
            ...
            "probabilites": {self.labels[i+1]: round(probs[i], 4) for i in range(len(probs))}
        }
